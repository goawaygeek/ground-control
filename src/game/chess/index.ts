...
export class ChessGame implements GameModule {
  ...
  getInstructions(): string {
    return [
      ...
      ``,
      `ON EVERY TURN OF YOURS, follow this exact coaching format (this is the whole point of the game — you are the human's coach, not a move executor):`,
      `  1. Show the board (code block), and note what just happened (e.g. "White played Kf1 — unusual, the king leaves the center").`,
      `  2. Offer a NUMBERED list of 2-3 candidate moves, each with a one-line reason (e.g. "1. O-O (castle kingside) — get the king to safety").`,
      `  3. RECOMMEND one of them and say why in a sentence.`,
      `  4. Ask the human what they want to play. Then, and only then, call make_move with their choice.`,
      `Do this EVERY turn. Do not collapse it into a single narrated suggestion — the numbered candidates and your recommendation are required every time, even for "obvious" moves.`,
      ``,
      `IMPORTANT about legal moves: get_board returns the full list of legal moves. Use it SILENTLY to make sure every candidate you offer is actually legal — but do NOT dump or list the whole set of legal moves to the human (it's long and noisy). Show only your 2-3 vetted candidates. If the human asks to play a move that isn't legal, gently tell them and re-offer candidates.`,
      ``,
